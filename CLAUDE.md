...
SENTINEL ✅ CLEAN — Per-execution quality gate + skill upgrade triggers
  ...
  Kimi remediation protocol (token-efficient — applies across ALL realms: marketing, product, strategy, intelligence, research, data narratives, internal ops):
    FLAG (1-2 eyes fail) → parent Claude agent patches failing sections only — no full rewrite. SENTINEL re-screens patched output.
    REJECT (3+ eyes fail) → parent Claude agent rewrites failing sections only, preserves passing sections. SENTINEL re-screens.
    If re-screen fails again → full Claude rewrite. Kimi output discarded.
    Rule: Claude touches the minimum required to reach Opus standard. Never rewrite what passed. No realm is exempt.
